...
module.exports = async function runTeleportEvent(page) {
  try {
    ...
    let tries = await page.$$eval(
      'div.currency-tries > span.currency-circle-full',
      els => els.length
    );
    ...
    while (tries > 0) {
      ...
      const sacrificialHint = new Set(
        [...hintSet].filter(h => bigBlock.has(h))
      );

      const laggingHint = new Set(
        [...hintSet].filter(h => !bigBlock.has(h))
      );

      const qualifiedUntouch = new Set(
        [...untouchedSet].filter(u => !bigBlock.has(u))
      );

      const highProb = new Set([
        ...laggingHint,
        ...qualifiedUntouch
      ]);
      ...
      if (hintArr.length > 0) {
        ...
        hintArr.forEach(hk => {
          ...
          const marks = getNeighbors(hr, hc)
            .filter(nk => highProb.has(key(nk)))
            .length;
        ...
        if (minMark <= 4 && hingeList.length) {
          const sac = hingeList.filter(h =>
            sacrificialHint.has(h.cell)
          );

          const chosenH = randomPick(
            sac.length ? sac : hingeList
          );

          const [hr, hc] = chosenH.cell
            .split(',')
            .map(Number);

          const targetNeighbors = getNeighbors(hr, hc)
            .filter(nk => highProb.has(key(nk)));
          ...
          if (targetNeighbors.length) {
            ...
            if (targetNeighbors.length === 1) {
              ...
              if (!sacrificialHint.has(chosenH.cell)) {
                const others = targetNeighbors.filter(
                  t => key(t) !== chosenH.cell
                );

                clickTarget = others.length
                  ? randomPick(others)
                  : randomPick(targetNeighbors);
              } else {
            ...
            await page.click(
              `.zone-grid > div > span:nth-child(${idx})`
            );
      ...
      if (!clicked && tries > 0) {
        const progress = await page.textContent(
          '.all-rewards-button-label'
        );

        const collected = parseInt(
          progress.split('/')[0].trim()
        );
      ...
      if (!clicked && tries > 0) {
        ...
        candidateHP.forEach(hk => {
          ...
          const overlap = getNeighbors(hr, hc)
            .filter(nk => bigBlock.has(key(nk)))
            .length;
      ...
      if (!clicked && tries > 0) {
        ...
        candidateHP.forEach(hk => {
          ...
          const overlap = getNeighbors(hr, hc)
            .filter(nk => laggingHint.has(key(nk)))
            .length;
