...
module.exports = async function runParties(page) {
  try {
    ...
    try {
      ...
      if (
        ...
        if (
          ...
          console.log(
            "🎁 Attendance bonus already collected."
          );
        } else {
          console.log(
            "⚠️ Attendance bonus could not be collected."
          );
        }
      }
    } catch (error) {
    ...
    try {
      ...
      // Fall through to direct getPartyTab() fallback.
    }

    // ------------------------------------------------------------
    // STEP 5C
    // Fallback: directly call getPartyTab()
    // ------------------------------------------------------------
